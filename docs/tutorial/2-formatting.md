...
## ページの設定 { #page-setup }
setルールの説明に戻ります。
setルールを書くときは、スタイルを設定したいエレメントの種類に応じて関数を選択します。
...
他のマークアップエレメントも同様に機能し、対応する関数呼び出しのための _シンタックスシュガー_ に過ぎません。
...
showルールを使用すると、Typstが特定のエレメントをどのように表示するかを再定義できます。
これにより、Typstがどのエレメントをどのように表示するかを指定します。
